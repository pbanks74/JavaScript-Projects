...
let activePlayer = 'X';
//Array stores an array of moves, used to determine win condidtions.
let selectedSquares = [];
...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        //This condition checks to see whos turn it is.
        if (activePlayer === 'X') {
            //If activePlayer is equal to 'X', the fire.png is placed in HTML.
            select.style.backgroundImage = 'url("images/fire.png")';
            //Active player may only be 'X' or 'O' so, if not 'X' it must be 'O'
        } else {
            //If activePlayer is equal to 'O', tree.png is placed in HTML.
            select.style.backgroundImage = 'url("images/tree.png")';
        }
        ...
        if (activePlayer ==='X') {
            ...
            activePlayer = 'O';
            //If activeis anything other than 'X'.
        } else {
        ...
        if(activePlayer === 'O') {
            //This function disables clicking foir computer choice.
            disableClick();
            //This function waits 1 second before computer places image and enables a click.
...
function checkWinConditions() {
    ...
    //squares are selected the code executes.
        ...
        audio('./media/farm_animals.mp3');
        //This function sets a .3 second timer before the restGame is called.
        setTimeout(function () { resetGame(); }, 1000);
    }

    //This functiuon checks if an array includes 3 strings. It is used to check for
    //each win condition.
    function arrayIncludes(squareA, squareB, squareC) {
        ...
        //If the 3 vaeriables we pass are all included in our array true is
        //returned and our else if condition executes ther drawWinLine function.
...
//This function takes a string parameter of the pth set earlier
//for placement sound.
function audio(audioURL) {
    //We create a new audio object and pass the path as a parameter.
    let audio = new Audio(audioURL);
    //Play method plays the aydio sound.
    audio.play();
}
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    y2 = coordY2,
    //Tihs variable stores temporary x axis data we update in our animation loop.
    x = xl,
...
function animateLineDrawing() {
    ...
    c.stroke();
    //This condition checks to see if we have reached the endpoint.
    if (xl <= x2 && yl <= y2) {
        //This condition adds 10 to the previous end x point.
        if (x < x2) { x += 10; }
        //This condition adds 10 to the previous end y point.
        if (y < y2) { y += 10; }
        //This condition canels the animation loop
        //if end points have been reached.
...
function resetGame() {
    ...
    //This resets the array so that it is empty and you can start over.
    selectedSquares = [];
} 